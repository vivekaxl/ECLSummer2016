...
IMPORT * FROM ParameterTuning;
 ...
 SWAY(DATASET(PLTypes.tuning_range_rec) t_ranges, REAL CF=0.75, REAL F=0.3) := MODULE  
        ...
        SHARED REAL real_random_between(INTEGER lower_limit, ANY upper_limit) := FUNCTION
                ...
                END;
        EXPORT DATASET(PLTypes.indep_sway_rec)  generate_population(INTEGER population_size=20):= FUNCTION
                ...
                RETURN independent_population;
        END;/*
        EXPORT DATASET(ML.Types.NumericField) run_one_generation(DATASET(ML.Types.NumericField) nf_zero_pop) := FUNCTION
                // Find poles
                ...
                // o_east := zero_pop[poles_indexes[1].x];
                // o_west := zero_pop[poles_indexes[1].y];
                c_2 := POWER(poles_indexes[1].value, 2);
                
                INTEGER projected(DATASET(Types.NumericField) east, DATASET(Types.NumericField) west, PLTypes.indep_sway_rec individual):= FUNCTION
                        t_individual := nf_zero_pop(id =individual.id);
                ...
                projected_population := SORT(PROJECT(zero_pop, TRANSFORM(add_proj_rec, 
                                                                                SELF.projected_distance := projected(east, west, LEFT);
                                                                                SELF.id := LEFT.id;
                                                                                )), projected_distance);

                split_point := COUNT(projected_population)/2;
                // first_split := projected_population[1..split_point];
                // second_split := projected_population[split_point+1..COUNT(projected_population)];
                // ASSERT(COUNT(first_split) = COUNT(second_split), 'The size of the splits should be equal');

                REAL ObjectiveFunction(PLTypes.indep_sway_rec individual):= FUNCTION
                                                RETURN individual.indep1 + individual.indep2 + individual.indep3 + individual.indep4 ;
                END;

                surviving_split := IF(ObjectiveFunction(o_east) >  ObjectiveFunction(o_west), first_split, second_split);
                surviving_split;
        END;*/
...
zero_pop := EA.generate_population(100);

// Change into Field
ToField(zero_pop, nf_zero_pop);
...
// Find poles
                ...
                // Sort is not required since surving_split.id is sorted
                DEDUP(surviving_split, surviving_split.id);
